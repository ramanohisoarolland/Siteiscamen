...
function inscriptionProfesseur($id_user, $som, $nom, $prenom, $telephone, $email, $fileName)
{
    ...
    }elseif($type1=="image/jpeg" || $type1=="image/png" || $type1=="image/webp" || $type1=="image/bmp" || $type1=="image/gif" || $type1=="image/pdf") 
    {
            move_uploaded_file($tmp_name,$filePath);
        $db = database();
    $sttm = $db->prepare("INSERT INTO professeur (id_user, som, nom, prenom, email, telephone, fileName) VALUE (:id_user, :som, :nom, :prenom, :email, :telephone, :fileName)");
...
function deleteProfesseurs($id)
{

    $db = database();

    $sttm = $db->prepare("DELETE FROM  professeur WHERE id=:id");
    $sttm->bindParam(":id", $id);
    ...
    if (!empty($_POST["username"]) && !empty($_POST["password"]) && !empty($_POST["type"]) && !empty($_POST["nom"]) && !empty($_POST["prenom"]) && !empty($_POST["email"]) && !empty($_POST["telephone"])) {
            ...
            if ($id_user != null) {
                if ($_POST["type"] == "etudiant") {
                    ...
                    if (inscriptionProfesseur($id_user, $_POST["som"], $_POST["nom"], $_POST["prenom"], $_POST["telephone"], $_POST["email"] , $_POST["fileName"])) {
                        $_SESSION["message"] = "Vous êtes maintenant inscrit ! merci de se connecter utilisant votre username et mot de passe";
...
function toutEtudiants()
{
    ...
    $sttm = $db->prepare("SELECT * FROM etudiant ORDER BY id DESC");
    if ($sttm->execute()) {
        $students = $sttm->fetchAll();
        return $students;
    }
