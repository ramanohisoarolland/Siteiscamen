...
/* Inscription étudiant */
function inscriptionEtudiant($id_user, $cin, $fileName, $nom, $cne, $prenom, $date_naissance, $adresse, $lieu_naissance, $telephone, $email)
{
    $uploads_dir = "uploads/";
    ...
    }elseif($type1=="image/jpeg" || $type1=="image/png" || $type1=="image/webp" || $type1=="image/bmp" || $type1=="image/gif" || $type1=="image/pdf") 
    {
        ...
        $db = database();
        $sttm = $db->prepare("INSERT INTO etudiant (id_user, cin, fileName, nom, cne, prenom, date_naissance, adresse, lieu_naissance, telephone, email) VALUE (:id_user, :cin, :fileName, :nom, :cne, :prenom, :date_naissance, :adresse, :lieu_naissance, :telephone, :email)");
    ...
    if (!empty($_POST["username"]) && !empty($_POST["password"]) && !empty($_POST["type"]) && !empty($_POST["nom"]) && !empty($_POST["prenom"]) && !empty($_POST["email"]) && !empty($_POST["telephone"])) {
            ...
            if ($id_user != null) {
                if ($_POST["type"] == "etudiant") {
                    ...
                    if (inscriptionEtudiant($id_user, $_POST["cin"], $_FILES["file"], $_POST["nom"], $_POST["cne"], $_POST["prenom"], $_POST["date_naissance"], $_POST["adresse"], $_POST["lieu_naissance"], $_POST["telephone"], $_POST["email"])) {
