...
function inscriptionEtudiant($id_user, $cin, $nom, $cne, $prenom, $date_naissance, $adresse, $lieu_naissance, $telephone, $email , $filiere, $niveau)
{

    $db = database();

    $sttm = $db->prepare("INSERT INTO etudiant (id_user, cin, nom, cne, prenom, date_naissance, adresse, lieu_naissance, telephone, email , filiere niveau) VALUE (:id_user, :cin, :nom, :cne, :prenom, :date_naissance, :adresse, :lieu_naissance, :telephone, :email , :filiere :niveau)");
...
function inscriptionProfesseur($id_user, $som, $fileName, $nom, $prenom, $telephone, $email)
{
    $uploads_dir = "uploads/";
    ...
    $sttm = $db->prepare("INSERT INTO professeur (id_user, som, fileName, nom, prenom, email, telephone) VALUE (:id_user, :som, :fileName, :nom, :prenom, :email, :telephone)");

    $sttm->bindParam(':id_user', $id_user);
    $sttm->bindParam(':fileName', $newName);
...
// inscription listModules

function inscriptionModule($id_user, $username, $nom_module,$nature)
{

    $db = database();

    $sttm = $db->prepare("INSERT INTO module (id_user, username, nom_module, nature) VALUE (:id_user, :username, :nom_module, :nature)");

    $sttm->bindParam(':id_user', $id_user);
    $sttm->bindParam(':username', $username);
    $sttm->bindParam(':nom_module', $nom_module);
    $sttm->bindParam(':nature', $nature);
...
function inscriptionEcolage($id_user, $mois, $date,$somme)
{

    $db = database();

    $sttm = $db->prepare("INSERT INTO ecolage (id_user, mois, date, somme) VALUE (:id_user, :mois, :date, :somme)");
    ...
    if (!empty($_POST["username"]) && !empty($_POST["password"]) && !empty($_POST["type"]) && !empty($_POST["nom"]) && !empty($_POST["prenom"]) && !empty($_POST["email"]) && !empty($_POST["telephone"])) {
            ...
            if ($id_user != null) {
                if ($_POST["type"] == "etudiant") {
                    ...
                    if (inscriptionEtudiant($id_user, $_POST["cin"], $_POST["nom"], $_POST["cne"], $_POST["prenom"], $_POST["date_naissance"], $_POST["adresse"], $_POST["lieu_naissance"], $_POST["telephone"], $_POST["email"] , $_POST["filiere"],  $_POST["niveau"])  && inscriptionEcolage($id_user, $_POST["mois"],  $_POST["date"], $_POST["somme"])) {
                    ...
                    if (inscriptionProfesseur($id_user, $_POST["som"], $_FILES["file"],  $_POST["nom"], $_POST["prenom"], $_POST["telephone"], $_POST["email"]) && inscriptionModule($id_user, $_POST["username"],  $_POST["nom_module"], $_POST["nature"])) {
                        $_SESSION["message"] = "Vous êtes maintenant inscrit ! merci de se connecter utilisant votre username et mot de passe";
                        header("Location: caisse.php");
                        exit();
                    }
...
function toutProfesseurs()
{
    ...
    $sttm = $db->prepare("SELECT * FROM professeur");
    if ($sttm->execute()) {
        $professeur = $sttm->fetchAll();
        return $professeur;
    }
...
function calculerProfesseur($id)
{
    ...
    $sttm = $db->prepare("SELECT count(id_user) FROM professeur where id_user=:id and is_old=0 ");
    $sttm->bindParam(':id', $id);
    if ($sttm->execute()) {
        $nombreProfesseur = $sttm->fetch(PDO::FETCH_ASSOC);
